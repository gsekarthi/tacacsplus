...
//  private byte HeaderFlags = HEADERFLAG_UNENCRYPT;
  ...
  public Tacacs(){
    ...
    hostname = "10.38.204.91";
    secretkey = "testing123".getBytes();
  }
  ...
  public synchronized boolean isAuthenticated(String Username, String Password) throws IOException, NoSuchAlgorithmException  {
    ...
    AS.authtype=AS.AUTHTYPE_MSCHAP;
    AS.send(Username,Password);
    AuthREPLY AR = new AuthREPLY();
    //System.err.println(AS.authtype==(byte)0x03);
    AR.get();
    boolean exitLoop = false;
    System.out.println("Status is :::: "+ " "+AR.getStatus());
    System.err.println(AR.getStatus());
    System.out.println("authtype"+AS.authtype);
    while (AR.getStatus() != AUTHEN_PASS && AR.getStatus() != AUTHEN_FAIL && AR.getStatus() != AUTHEN_ERROR && AR.getStatus() != AUTHEN_FOLLOW && exitLoop != true) {
      ...
      System.out.println("Tacacs.isAuthenticated()"+(AR.REPLY_status ));
      if (AR.REPLY_status == AUTHEN_GETDATA | AR.REPLY_status == AUTHEN_GETUSER) {
        ...
        AR.get();
    	  ...
    	  System.out.println("Insaide else if getPass");
        AuthCONT AC = new AuthCONT();
        AC.send(Password);
        AR.get();
      }
    ...
    System.out.println("Reply is::::: "+AR.toString());
  ...
  private class AuthSTART {
    ...
    private byte authtype = AUTHTYPE_PAP;
    private byte privlvl = PRIVLVL_MIN;
    private byte service = SERVICE_ENABLE;//SERVICE_LOGIN;//SERVICE_NONE;

    private AuthSTART() {
    	System.err.println("authtype is::: "+this.authtype);
    }
    private void send(String User, String Pass) throws IOException, NoSuchAlgorithmException {
      byte[] Username = User.getBytes();
      byte[] Data = Pass.getBytes();
      byte[] Port = "19230".getBytes();
      byte[] RemoteAdd = hostname.getBytes();
      System.err.println("Addr is :::::: "+Arrays.toString(RemoteAdd));
      ...
      byte[] body = Header.crypt(version.byteValue(), tacacsSequence.byteValue(), 
            baos.toByteArray(),headerFlags, sessionID, secretkey);
      baos.reset();
      byte[] header = Header.makeHeader(body,version,Header.TYPE_AUTHENTIC,
  ...
  private class AuthCONT {
    ...
    private void send(String UserMsgData) throws IOException, NoSuchAlgorithmException {
      ...
      baos.write(header);
      baos.write(body);
      System.out.println("Writting:::: "+Arrays.toString(body));
      baos.writeTo(theSocket.getOutputStream());
    }
  }
  private class AuthREPLY {
	...
	public String toString() {
		return "AuthREPLY [FLAG_NOECHO=" + FLAG_NOECHO + ", REPLY_status=" + REPLY_status + ", REPLY_flags="
				+ REPLY_flags + ", servermsgLen=" + Arrays.toString(servermsgLen) + ", dataLen="
				+ Arrays.toString(dataLen) + ", getStatus()=" + getStatus() + ", getFlags()=" + getFlags()
    ...
    private void get() throws IOException,SocketException,NoSuchAlgorithmException {
      ...
      System.out.println("bous is ::: "+baos);
      ...
      System.out.println("2nd baos is :::: "+baos);
      ...
      System.out.println("header is ::: "+headerFlags);
      ...
      body = Header.crypt(headerVersionNumber,headerSequenceNumber,tempBody,headerFlags,
            sessionID,secretkey);
      System.out.println("body is ::::::: "+Arrays.toString(body));
      for (byte b : body) {
    	  System.out.println(b);
      }
      REPLY_status = body[0];
      System.out.println("reply status is :::: "+REPLY_status);
      REPLY_flags = body[1];
    }
...
public static void main(String[] args) {
	Tacacs tc = new Tacacs();
	tc.new AuthSTART().authtype=(byte)0x04;
	tc.setPortNumber(19230);
	try {
		System.out.println("Authentrication is :::::: "+tc.isAuthenticated("qatester", "qatest123"));
